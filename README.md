...
**TODO:** The code as designed only allows one cofactor and one ligand at a time; a useful extension would be to allow an arbitrary number of ligands or cofactors for each fold.
...
## Analysis
The idea behind the scripts in the `analysis` directory is to use metrics generated by AlphaFold, plus RMSD, to identify high-likelihood protein interactions. This approach should be used with caution -- please see the [CoIP notebooks](https://github.com/photosynthesharpe/alpha-utility/blob/main/notebooks/coIP_vs_predicted.ipynb) for an analysis of these metrics.
